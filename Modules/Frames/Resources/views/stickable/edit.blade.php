        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <div class="box box-danger">
                    ...
                    <!-- form start -->
                    {!! Form::open(['route' => ['stickableframe.update', $stickable->id], 'method' => 'PUT', 'role' => 'form', 'autocomplete' => 'off', 'enctype' => 'multipart/form-data', 'id' => 'formFrames']) !!}
                    <div class="box-body">

                        <div class="form-group {{ $errors->first('title') ? 'has-error' : '' }}">
                            <label for="ftitle">Title <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="ftitle" placeholder="Title"
                                name="title" value="{{ old('title', $stickable->title) }}" required
                        ...
                        <div class="form-group {{ ($errors->first('slug')) ? 'has-error' : '' }}">
                            ...
                            <input type="text" class="form-control" id="fslug" placeholder="Slug" name="slug" required 
                                value="{{ old('slug', $stickable->slug) }}"
                                data-parsley-trigger="keyup focusout"
                          ...
                          <div class="form-group {{ ($errors->first('class')) ? 'has-error' : '' }}">
                            ...
                            value="{{ old('class', $stickable->class) }}" required
                              data-parsley-trigger="keyup focusout">
                                            @if($errors->has('class'))										
                          ...
                          <div class="form-group {{ ($errors->first('image')) ? 'has-error' : '' }}">
                            ...
                            <div style="margin-top: 5px;">                          
                              <img src="{{Storage::url(old('image', $stickable->image))}}" alt="setting" height="100" id="image">
                            </div>
                            @if($errors->has('image'))                    
                          ...
                          <div class="form-group {{ ($errors->first('order')) ? 'has-error' : '' }}" >
                            ...
                            <input type="numeric" class="form-control input-price" id="forder" placeholder="Order" name="order" 
                              value="{{ old('order', $stickable->order) }}" 
                              data-parsley-trigger="keyup focusout"
                          ...
                          <div class="form-group {{ ($errors->first('status')) ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="fstatus" required data-parsley-trigger="keyup focusout">
                              ...
                              @foreach(config('frames.publish_status') as $status)
                                <option value="{{ $status['value'] }}" {{ (old('status', $stickable->status)==$status['value']) ? 'selected' : '' }}>{{ $status['name'] }}</option>
                              @endforeach
                            </select>
                    ...
                    <div class="box-footer">
                        <div class="pull-right">
                            <a href="{{ route('stickableframe.index') }}" class="btn btn-info"><i class="glyphicon glyphicon-backward"></i> Back</a>
                        ...
                        <table class="table">
                            <tbody>
                                <tr>
                                    ...
                                    <td>: {{@$stickable->created_by->name}}</td>
                                </tr>
                                <tr>
